...
module.exports = ASQPlugin.extend({
  ...
  answerSubmission: coroutine(function *answerSubmissionGen (answer){
    ...
    this.sendCodeToJavabox('hello', answer.submission, function(result){
      console.log(result);
    });
  ...
  onPlugin: function onPlugin (evt){
    console.log('HHHHHIIIIIIIII' + evt);
    if(evt.type !== 'asq-java-q') return;

    evt.type = 'output';
    evt.questionType = this.tagName;

    this.asq.socket.emit('asq:question_type', evt, evt.socketId);
  ...
  sendCodeToJavabox(fileName, code, cb){
    // todo:
